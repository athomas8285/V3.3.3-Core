...
function buildCharts(card,mid){
  ...
  var _bd=cs.getPropertyValue('--bd').trim()||'#e8e6dc';
  var _ink=cs.getPropertyValue('--ink').trim()||'#1B365D';
  var _red=cs.getPropertyValue('--red').trim()||'#b05a5a';
  var _amber=cs.getPropertyValue('--amber').trim()||'#b8943a';
  var _green=cs.getPropertyValue('--green').trim()||'#4a7a5a';
  var _t1=cs.getPropertyValue('--t1').trim()||'#141413';
  var _t2=cs.getPropertyValue('--t2').trim()||'#504e49';
  var _t3=cs.getPropertyValue('--t3').trim()||'#6b6a64';
  ...
  for(var gi=0;gi<=4;gi++){
    ...
    gl.setAttribute('stroke',_bd);gl.setAttribute('stroke-width','1');s1.appendChild(gl);
    var gt=document.createElementNS(NS,'text');
    gt.setAttribute('x','28');gt.setAttribute('y',y+3);gt.setAttribute('fill',_t3);
  ...
  function mb(x,y,w,h,c,v,l){
    ...
    t.setAttribute('x',x+w/2);t.setAttribute('y',y-5);t.setAttribute('fill',_t1);
    ...
    lb.setAttribute('x',x+w/2);lb.setAttribute('y',cY+cH+11);lb.setAttribute('fill',_t3);
    lb.setAttribute('font-size','11');lb.setAttribute('text-anchor','middle');lb.textContent=l;s1.appendChild(lb);
  }
  mb(bX1,cY+cH-Math.max((lh/maxV)*cH,3),bW,Math.max((lh/maxV)*cH,3),_ink,lh.toFixed(3),'\u03bb\u4e3b');
  mb(bX1+bW+gap,cY+cH-Math.max((la/maxV)*cH,3),bW,Math.max((la/maxV)*cH,3),_red,la.toFixed(3),'\u03bb\u5ba2');
  var tt=document.createElementNS(NS,'text');
  tt.setAttribute('x','32');tt.setAttribute('y','13');tt.setAttribute('fill',_amber);
  ...
  var dc=Math.abs(dH)>0.05?_red:_green;
  ...
  for(var pi=0;pi<=maxP*100;pi+=10){
    ...
    ln.setAttribute('stroke',_bd);ln.setAttribute('stroke-width','1');s2.appendChild(ln);
    var tx=document.createElementNS(NS,'text');
    tx.setAttribute('x','28');tx.setAttribute('y',y2+3);tx.setAttribute('fill',_t3);
  ...
  groups.forEach(function(g,i){
    ...
    rp.setAttribute('rx','2');rp.setAttribute('fill',_ink);rp.setAttribute('opacity','0.85');s2.appendChild(rp);
    var tv1=document.createElementNS(NS,'text');
    tv1.setAttribute('x',x+barW/2);tv1.setAttribute('y',cY2+cH2-b1-4);
    tv1.setAttribute('fill',_ink);tv1.setAttribute('font-size','11');tv1.setAttribute('font-weight','600');
    ...
    rm.setAttribute('rx','2');rm.setAttribute('fill',_red);rm.setAttribute('opacity','0.8');s2.appendChild(rm);
    var tv2=document.createElementNS(NS,'text');
    tv2.setAttribute('x',x2+barW/2);tv2.setAttribute('y',cY2+cH2-b2-4);
    tv2.setAttribute('fill',_red);tv2.setAttribute('font-size','11');tv2.setAttribute('font-weight','600');
    ...
    lbl.setAttribute('fill',_t3);lbl.setAttribute('font-size','11');lbl.setAttribute('text-anchor','middle');
    ...
    if(i===0&&Math.abs(g[1]-g[2])>0.02){
      ...
      ar.setAttribute('fill',_amber);ar.setAttribute('font-size','10');ar.setAttribute('font-weight','700');ar.setAttribute('text-anchor','middle');
  ...
  lg1.setAttribute('fill',_ink);lg1.setAttribute('font-size','10');lg1.setAttribute('font-weight','500');
  ...
  lg2.setAttribute('fill',_red);lg2.setAttribute('font-size','10');lg2.setAttribute('font-weight','500');
...
function rs(d){
  ...
  var h='<div class="sec"><span class="sec-dot"></span><span class="sec-h">今日扫盘</span>' +'<select id="sortSelect" style="margin-left:12px;background:var(--surface-2);border:1px solid var(--bd);border-radius:4px;padding:2px 6px;font-size:11px;color:var(--t2);font-family:var(--sans);outline:none;cursor:pointer" onchange="SORT_BY=this.value;if(_DATA)rs(_DATA);">' +'<option value="id"'+(SORT_BY==='id'?' selected':'')+'>编号</option>' +'<option value="fit"'+(SORT_BY==='fit'?' selected':'')+'>推荐度</option>' +'</select>' +'<div style="position:relative;margin-left:8px;display:inline-block">' +'<span onclick="toggleFilter()" style="cursor:pointer;background:var(--surface-2);border:1px solid var(--bd);border-radius:4px;padding:2px 8px;font-size:11px;color:var(--t2);font-family:var(--sans);white-space:nowrap;user-select:none">筛选 ▼</span>' +'<div id="filterDropdown" style="display:none;position:absolute;top:100%;left:0;margin-top:4px;background:var(--surface);border:1px solid var(--bd);border-radius:8px;padding:10px;z-index:50;min-width:160px;box-shadow:0 6px 24px rgba(0,0,0,0.12)">' +'<div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px;letter-spacing:0.3px">推荐等级</div>' +['S','A','B','C'].map(function(l){return'<label style="display:flex;align-items:center;gap:6px;padding:4px 6px;cursor:pointer;font-size:12px;color:var(--t2);border-radius:4px;transition:background 0.15s"><input type="checkbox" '+(FILTER.rating[l]?'checked':'')+' onchange="setFilter(\'rating\',\''+l+'\')" style="margin:0"><span>'+l+'级</span></label>';})+'<div style="border-top:1px solid var(--bd);margin:6px 0 4px;padding-top:6px">'+'<div style="font-size:10px;color:var(--t3);font-weight:600;margin-bottom:4px;letter-spacing:0.3px">赛事类型</div>'+'<label style="display:flex;align-items:center;gap:6px;padding:4px 6px;cursor:pointer;font-size:12px;color:var(--t2);border-radius:4px;transition:background 0.15s"><input type="checkbox" '+(FILTER_ALL_EVENTS?'checked':'')+' onchange="setFilter(\'event_all\',\'all\')" style="margin:0"><span>全部赛事</span></label>'+_DATA.rating.reduce(function(acc,x){var ev=x.event||'';if(ev&&acc.indexOf(ev)<0)acc.push(ev);return acc;},[]).map(function(ev){return'<label style="display:flex;align-items:center;gap:6px;padding:4px 6px;cursor:pointer;font-size:12px;color:var(--t2);border-radius:4px;transition:background 0.15s"><input type="checkbox" '+(FILTER_ALL_EVENTS||FILTER.event[ev]?'checked':'')+' onchange="setFilter(\'event\',\''+ev+'\')" style="margin:0"><span>'+ev+'</span></label>';}).join('')+'</div>'+'</div></div>' +'<span class="sec-l"></span></div>';
h+='<div class="table-scroll"><div class="thr"><span>赛事编号</span><span>开始时间</span><span>主队vs客队</span><span>胜平负</span><span>总进球</span><span>半全场</span><span>比分</span><span>推荐度</span></div></div>';
...
function renderReview(){
  ...
  if(yr.length){
    h+='<div class="table-scroll"><div class="thr"><span class="c2-id">赛事编号</span><span class="c2-tm">开始时间</span><span class="c2-mm">主队vs客队</span><span class="c2-dr">胜平负</span><span style="width:180px;text-align:center">赛果</span><span style="width:90px;text-align:center;padding-left:40px">命中</span></div></div>';
    yr.forEach(function(m){
      ...
      h+=['<div class="c2-col c2-id"><span class="c-id">#'+m.id+'</span></div>',
          ...
          '<div class="c2-col" style="width:180px;text-align:center;font-family:var(--mono);font-size:14px;font-weight:600;color:'+scCol+'">'+score+'/'+ht+'</div>',
