  ...
  function loadLiveInfo() {
    ...
    fetch('/api/live/info').then(function(r){return r.json();}).then(function(data){
      if(!data.matches||data.matches.length===0){
        body.innerHTML='<div style="text-align:center;padding:30px;color:var(--t3);font-size:12px">no matches</div>';
        return;
      }
      var h='';
      data.matches.forEach(function(m){
        ...
        h+='<div style="font-size:10px;color:var(--t3);margin:2px 0 6px">'+m.event+' '+m.time.slice(5,16)+' | init: '+initSP+'</div>';
        h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;margin-bottom:6px">';
        h+='<div><label style="font-size:10px;color:var(--t3)">home</label><br><input id="sp_h_'+m.id+'" value="'+m.sp_home+'" style="width:100%;padding:3px 6px;border:1px solid var(--bd);border-radius:3px;font-family:var(--mono);font-size:12px;background:var(--surface)"></div>';
        h+='<div><label style="font-size:10px;color:var(--t3)">draw</label><br><input id="sp_d_'+m.id+'" value="'+m.sp_draw+'" style="width:100%;padding:3px 6px;border:1px solid var(--bd);border-radius:3px;font-family:var(--mono);font-size:12px;background:var(--surface)"></div>';
        h+='<div><label style="font-size:10px;color:var(--t3)">away</label><br><input id="sp_a_'+m.id+'" value="'+m.sp_away+'" style="width:100%;padding:3px 6px;border:1px solid var(--bd);border-radius:3px;font-family:var(--mono);font-size:12px;background:var(--surface)"></div>';
        h+='</div>';
        h+='<div id="live_result_'+m.id+'"></div>';
        h+='<div style="margin-top:4px;display:flex;gap:4px">';
        h+='<button onclick="doLiveRecalibrate(\''+m.id+'\')" style="flex:1;background:var(--green);color:#fff;border:none;padding:4px 8px;border-radius:3px;font-size:11px;cursor:pointer">recalibrate</button>';
        h+='<button onclick="setLineup(\''+m.id+'\',true)" style="background:#eef2f7;color:var(--ink);border:1px solid var(--bd);padding:4px 8px;border-radius:3px;font-size:11px;cursor:pointer">XI yes</button>';
        h+='<button onclick="setLineup(\''+m.id+'\',false)" style="background:#f5ecec;color:var(--red);border:1px solid var(--bd);padding:4px 8px;border-radius:3px;font-size:11px;cursor:pointer">key out</button>';
        h+='</div></div>';
      });
      h+='<button onclick="doBatchRecalibrate()" style="width:100%;background:var(--ink);color:#f5f4ed;border:none;padding:6px;border-radius:4px;font-size:12px;cursor:pointer;margin-top:4px">batch all</button>';
      body.innerHTML=h;
    }).catch(function(){
      body.innerHTML='<div style="text-align:center;padding:30px;color:var(--red);font-size:12px">load failed</div>';
    });
  }
  ...
  window.doLiveRecalibrate = function(mid) {
    ...
    fetch('/api/live/recalibrate',{
      ...
      if(!result||result.error){
        document.getElementById('live_result_'+mid).innerHTML='<div style="color:var(--red);font-size:11px">err: '+(result?result.error:'unknown')+'</div>';
        return;
      }
      ...
      html+='<div style="display:flex;justify-content:space-between;font-weight:600;color:var(--t2);border-bottom:1px solid var(--bd);padding-bottom:3px;margin-bottom:3px"><span>before</span><span>after</span></div>';
      html+='<div style="display:flex;justify-content:space-between"><span>direction</span><span>'+b.direction+'</span><span>'+a.direction+'</span></div>';
      html+='<div style="display:flex;justify-content:space-between"><span>fit</span><span>'+b.fit_score.toFixed(1)+'</span><span>'+a.fit_score.toFixed(1)+' ('+(fitDelta>0?'+':'')+fitDelta+')</span></div>';
      html+='<div style="display:flex;justify-content:space-between"><span>rating</span><span>'+b.rating+'</span><span>'+a.rating+'</span></div>';
      html+='<div style="display:flex;justify-content:space-between"><span>DDI home</span><span>'+ddiH.old.toFixed(4)+'</span><span>'+ddiH.new.toFixed(4)+' ('+(ddiH.delta>0?'+':'')+ddiH.delta.toFixed(4)+')</span></div>';
      ...
      document.getElementById('live_result_'+mid).innerHTML='<div style="color:var(--red);font-size:11px">request failed</div>';
    });
  };
  ...
  window.setLineup = function(mid, confirmed) {
    ...
    fetch('/api/live/recalibrate',{
      ...
      if(result&&!result.error){
        ...
        var html='<div style="background:var(--surface-2);border-radius:4px;padding:6px 8px;font-size:11px;line-height:1.6">';
        html+='<div style="color:var(--green);font-weight:600;margin-bottom:3px">'+(confirmed?'XI confirmed':'key out')+'</div>';
