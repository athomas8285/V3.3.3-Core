...
function dirTag(d){
  if(d.indexOf('\u4e3b')>=0)return'<span class="mc-dir dir-h">\u4e3b\u80dc</span>';
  if(d.indexOf('\u5ba2')>=0)return'<span class="mc-dir dir-a">\u5ba2\u80dc</span>';
  if(d.indexOf('\u5e73')>=0)return'<span class="mc-dir dir-d">\u5e73\u5c40</span>';
  return'<span class="mc-dir">'+d+'</span>';
}
function gr(ra,fs){
...
function buildSummary(d){
  ...
  var sh='<b>'+t+'</b> \u573a &emsp; \u5e73\u5747 <b>'+avg+'</b>';
  var cl={S:'var(--green)',A:'var(--accent)',B:'var(--amber)',C:'var(--t3)'};
  ['S','A','B','C'].forEach(function(k){if(c[k]>0)sh+=' &emsp; <span style="color:'+cl[k]+';font-weight:600">'+k+'<b>'+c[k]+'</b></span>';});
  // date from first match
  var td=new Date();var dt=td.getFullYear()+"."+(td.getMonth()+1)+"."+td.getDate();window._statsText=dt+" 扫盘 "+sh;
  var rd='',cols={S:'rgba(34,214,94,0.35)',A:'rgba(14,165,233,0.35)',B:'rgba(245,165,36,0.35)',C:'rgba(74,79,106,0.4)'};
  ...
  var rdEl=document.getElementById('rd');if(rdEl)rdEl.innerHTML=rd;
}
function buildCharts(card,mid){
  ...
  for(var gi=0;gi<=4;gi++){
    ...
    gt.setAttribute('style','font-family:var(--mono)');
  ...
  function mb(x,y,w,h,c,v,l){
    ...
    t.setAttribute('style','font-family:var(--mono)');t.textContent=v;s1.appendChild(t);
  ...
  tt.setAttribute('style','font-family:var(--mono)');tt.textContent='\u0394 '+ld.toFixed(3);s1.appendChild(tt);
  ...
  dt.setAttribute('style','font-family:var(--mono)');
  ...
  for(var pi=0;pi<=maxP*100;pi+=10){
    ...
    tx.setAttribute('style','font-family:var(--mono)');tx.textContent=pi+'%';s2.appendChild(tx);
  }
  ...
  groups.forEach(function(g,i){
    ...
    tv1.setAttribute('text-anchor','middle');tv1.setAttribute('style','font-family:var(--mono)');
    ...
    tv2.setAttribute('text-anchor','middle');tv2.setAttribute('style','font-family:var(--mono)');
    ...
    dt2.setAttribute('style','font-family:var(--mono)');
    ...
    if(i===0&&Math.abs(g[1]-g[2])>0.02){
      ...
      ar.setAttribute('style','font-family:var(--mono)');
  ...
  lg1.setAttribute('style','font-family:var(--mono)');lg1.textContent='\u25a0 \u7269\u7406';s2.appendChild(lg1);
  ...
  lg2.setAttribute('style','font-family:var(--mono)');lg2.textContent='\u25a0 \u5e02\u573a';s2.appendChild(lg2);
...
function rs(d){
  ...
  s.forEach(function(x){
    ...
    h+='<div class="match-card "+rtClass(x.rating,x.fit_score)+"\" data-mid=\""+mid+"\">';
    h+='<div class="mc-row1">';
    h+='<div class="mc-left">';
    h+='<span class="mc-meta">#'+mid+' '+ko+'</span>';
    h+='<span class="mc-teams">'+x.home+'<span class="mc-vs">vs</span>'+x.away+'</span>';
    if(st)h+='<span class="mc-tag">'+st+'</span>';
    h+='</div>';
    h+='<div class="mc-right">';
    h+=dirTag(x.direction);
    h+='<span class="mc-score">'+x.fit_score.toFixed(1)+' '+badge(x.rating,x.fit_score)+'</span>';    h+='<span class="mc-arrow">\u25bc</span>';    h+='<span class="mc-arrow">\u25bc</span>';
    h+='</div>';
    h+='</div>';
    h+='<div class="mc-row2">';
    h+='<span class="mc-ds">'+gs+'</span>';
    h+='<span class="mc-sep">|</span>';
    h+='<span class="mc-ds">'+hs2+'</span>';
    h+='<span class="mc-sep">|</span>';
    h+='<span class="mc-ds">'+ss+'</span>';
    h+='<span class="mc-sep">|</span>';
    h+='<span class="mc-ds">\Λ '+lh.toFixed(2)+'/'+la.toFixed(2)+'</span>';
    h+='<span class="mc-sep">|</span>';
    h+='<span class="mc-ds '+dCls+'">DDI '+(dh>=0?'+':'')+dh.toFixed(3)+'</span>';
    h+='<span class="mc-sep">|</span>';
    h+='<span class="mc-ds">\\u76d8\\u53e3 '+hs+'</span>';
    h+='</div>';
  ...
  var si=document.getElementById('statsInline');if(si&&window._statsText)si.innerHTML=window._statsText;
  document.querySelectorAll('.match-card').forEach(function(c){
    c.addEventListener('click',function(){
      var e=this.classList.contains('active');
      document.querySelectorAll('.match-card.active').forEach(function(x){x.classList.remove('active');});
      if(!e){this.classList.add('active');buildCharts(this,this.getAttribute('data-mid'));}
    });
  });
}
...
function renderReview(){
  ...
  if(yr.length){var ym={};yr.forEach(function(m){var pd=m.prediction_date||m.date.substring(0,10);ym[pd]=(ym[pd]||0)+1;});}
  if(yr.length){var ym={};yr.forEach(function(m){var pd=m.prediction_date||m.date.substring(0,10);ym[pd]=(ym[pd]||0)+1;});}
  var ds=Object.keys(ym).sort().reverse();var ys=ds[0];
  yr=RV.filter(function(m){var mpd=m.prediction_date||m.date.substring(0,10);return mpd===ys;});
  ...
  if(yr.length){
    yr.forEach(function(m){
      ...
      h+='<div class="match-card "+rt+"\">';
      h+='<div class="mc-row1">';
      h+='<div class="mc-left">';
      h+='<span class="mc-meta">#'+m.id+' '+ko+'</span>';
      h+='<span class="mc-teams">'+m.home+'<span class="mc-vs">vs</span>'+m.away+'</span>';
      h+='</div>';
      h+='<div class="mc-right">';
      h+='<span style="color:'+scCol+'">'+dTag+'</span>';
      h+='<span class="mc-score">'+(m.fit_score?m.fit_score.toFixed(1):'--')+' '+(m.rating?'<span class="badge badge-'+m.rating.toLowerCase()+'">'+m.rating+'</span>':'')+'</span>';
      h+='<span class="mc-arrow">▼</span>';

      h+='</div>';
      h+='</div>';
      h+='<div class="mc-row2">';
      h+='<span class="mc-ds" style="color:'+scCol+';font-weight:600">'+score+'/'+ht+'</span>';
      h+='<span class="mc-sep">|</span>';
      h+='<span class="mc-ds"><span style="display:inline-block;background:'+(hit?'#ef5350':'var(--t3)')+';color:#fff;font-size:10px;padding:1px 8px;border-radius:3px;font-weight:600">'+(hit?'\u547d\u4e2d':'\u672a\u4e2d')+'</span></span>';
      h+='</div>';
  ...
  document.querySelectorAll('#rv .match-card').forEach(function(c){
    c.addEventListener('click',function(){
      var e=this.classList.contains('active');
      document.querySelectorAll('#rv .match-card.active').forEach(function(x){x.classList.remove('active');});
      if(!e){this.classList.add('active');}
    });
  });
}

window.onload=function(){
  fetch('/api/v2/data').then(function(r){return r.json();}).then(function(d){
    var ts=d.today_scan;
    if(ts&&ts.status==='ACTIVE'&&typeof rs==='function'){rs(d);}
    else if(ts&&ts.status==='IDLE'){if(document.getElementById('todayContent'))document.getElementById('todayContent').innerHTML='<div class=idle-state><span class=idle-dot></span><span>等待今日预测任务</span></div>';}
    else if(d.rating&&d.rating.length>0)rs(d);
  }).catch(function(){});
};
